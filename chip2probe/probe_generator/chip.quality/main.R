...
imads_site <- read.imads.bed(bed_path)
...
for (len in peaklen){
  ...
  peak_sites <- merge(x = site_count, y = agg_both, by = c("chr", "peak.start", "peak.end"))
  sites_ctpile <- data.frame(count = merged$site.count, pileup = merged$pileup.score)
  ...
  setkey(peak_sites, "chr", "peak.start","peak.end")

  for (ct in count_sites_per_peak){
    peaks_selected <- peak_sites[peak_sites$site.count == ct,] %>% select(-site.count) # count is not needed after this
