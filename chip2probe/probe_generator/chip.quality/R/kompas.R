...
match_fwd <- lapply(head(fwds,5), kmer.match, escore_df = align_df, k = k)
center_fwd <- mapply(find.center, match_fwd, orient="fwd", head(seq_lens,5), center_pos=center_pos, threshold=threshold)
sites_fwd <- rbindlist(mapply(get.sites, center_fwd, head(peak_list,5), orient='fwd', SIMPLIFY=FALSE)) %>%
             filter_all(all_vars(!is.na(.)))

match_rc <- lapply(head(rev_comps,5), kmer.match, escore_df = align_df, k = k)
center_rc <- mapply(find.center, match_rc, orient="rc", head(seq_lens,5), center_pos=center_pos, threshold=threshold)
sites_rc <- rbindlist(mapply(get.sites, center_rc, head(peak_list,5), orient='rc', SIMPLIFY=FALSE)) %>% 
             filter_all(all_vars(!is.na(.)))
...
get.sites <- function(center_list, peak_coord, orient){
  #if(nrow) peak_coord
  seqlen <- peak_coord$end - peak_coord$start + 1
  centers <- unlist(center_list)
  start_pos <- if(orient == "fwd") peak_coord$start + centers else peak_coord$start + seqlen - centers - motif_len
  ori <- if_else(orient == "fwd", "+", "-")
  sites <- data.frame("chr"= as.character(peak_coord$chr), "start"=start_pos, "end"=start_pos+motif_len,'ori'=ori)
  return(sites)
}
