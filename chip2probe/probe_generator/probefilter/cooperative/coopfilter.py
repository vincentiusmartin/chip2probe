...
def check_sequence(seq, sites_list, pbmescores, escore_cutoff=0.4, escore_gap=0):
    """
...
def check_all_seqs(wt, m1, m2, m3, sites_dict, pbmescores, escore_cutoff=0.4,
    ...
    wt_cond = check_sequence(wt, [s1, s2], pbmescores, escore_cutoff, escore_gap)
    # for m1, only s2 is specific
    m1_cond = check_sequence(m1, [s2], pbmescores, escore_cutoff, escore_gap)
    # for m2, only s1 is specific
    m2_cond = check_sequence(m2, [s1], pbmescores, escore_cutoff, escore_gap)
    # for m3, both sites are non-specific
    m3_cond = check_sequence(m3, [], pbmescores, escore_cutoff, escore_gap)
