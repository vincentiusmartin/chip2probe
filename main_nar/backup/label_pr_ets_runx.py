...
def get_normalization_coefs(df1, df2, predictor):
    ...
    arr.plot_chamber_corr(med1, med2, median=False,
                           ...
                           title="Correlation between chambers", xlab="Ets1", ylab="Ets1 + Runx1", valcol="intensity")
...
def get_negcutoff(negdf_m, negdf_mc, tf_str, slope, intercept, percentile):
    # median -- group by orientation replicates, take median
    negdf_m = negdf_m[negdf_m["Name"].str.contains(tf_str)]

    # negdf_mc = negdf_mc[negdf_mc["Name"].str.contains(tf_str)]
    # negdf_mc["unnormalized_intensity"]  = negdf_mc["intensity"]
    # negdf_mc["intensity"]  = list(((negdf_mc["intensity"] - intercept)/slope).values)
    #
    # nd = negdf_m.merge(negdf_mc, on=["Name","Sequence","rep","ori"])
    # nd = nd[["intensity_x", "intensity_y","unnormalized_intensity"]].reset_index()
    # nd = nd.rename(columns={"intensity_x":"Ets1", "unnormalized_intensity": "Ets1+Runx1_unnormalized","intensity_y":"Ets1+Runx1_normalized"})
    #
    # nd["id"] = nd.index + 1
    # nd["id"] =  nd["id"].apply(lambda x: "negctrl_seq_%s" % x)
    nd.to_csv("negdf.csv",index=False)
...
if __name__ == "__main__":
    ...
    slope,intercept = 0.82,123.36
    print("Normalization using y = %.2fx + %.2f" % (slope,intercept))

    cutoff = float(get_negcutoff(neg_m, neg_mc, cooptf, slope, intercept, neg_percentile))
    # cutoff = 412.568
    ...
    both_ori.sort_values("Name").to_csv("%s/both_%s_%s.csv" % (baseoutpath,maintf,cooptf), index=False)
    ...
    both_ori_plt[both_ori_plt["label"] != "below_cutoff"].sort_values("Name").to_csv("%s/both_ori_plt_%s_%s.csv" % (baseoutpath,maintf,cooptf),index=False)
    arr.plot_classified_labels(both_ori_plt, path="%s/both_normalized_%s_%s_unnormalzed.png" % (baseoutpath,maintf,cooptf), col1="intensity_x", col2="unnormalized_intensity", title=both_title, xlab=ch_x, ylab=ch_y, plotnonsignif=False, labelnames=["cooperative","independent","anticoop"], colors=colors)
    arr.plot_classified_labels(both_ori_plt, path="%s/both_normalized_%s_%s.png" % (baseoutpath,maintf,cooptf), col1="intensity_x", col2="intensity_y",
            title=both_title, xlab=ch_x, ylab=ch_y, plotnonsignif=False, labelnames=["cooperative","independent","anticoop"], colors=colors)
