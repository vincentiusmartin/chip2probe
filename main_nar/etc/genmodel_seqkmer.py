...
if __name__ == "__main__":
    ...
    rf_param_grid = {
        'n_estimators': [500,1000], #[500,750,1000],
        'max_depth': [5,10], #[5,10,15],
        "min_samples_leaf": [5,10], #[5,10,15],
        "min_samples_split" : [5,10] #[5,10,15]
    }
        ...
        "1":
            BestModel(clf="sklearn.ensemble.RandomForestClassifier",
              ...
              train_data=ct.get_training_df({
                ...
                "sequence_in":{"seqin":4, "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1},
                "sequence_out":{"seqin":-1, "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1}
        ...
        "2":
            BestModel(clf="sklearn.ensemble.RandomForestClassifier",
              ...
              train_data=ct.get_training_df({
                    ...
                    "orientation": {"relative":rel_ori, "one_hot":one_hot_ori, "pos_cols": {"%s_pos"%s1:"%s_ori"%s1, "%s_pos"%s2:"%s_ori"%s2}},
                    "sequence_in":{"seqin":4, "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1},
                    "sequence_out":{"seqin":-2, "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1}
                }, label_map={'cooperative': 1, 'independent': 0})
            ).run_all(),
        "3":
            BestModel(clf="sklearn.ensemble.RandomForestClassifier",
              ...
              train_data=ct.get_training_df({
                    ...
                    "orientation": {"relative":rel_ori, "one_hot":one_hot_ori, "pos_cols": {"%s_pos"%s1:"%s_ori"%s1, "%s_pos"%s2:"%s_ori"%s2}},
                    "sequence_in":{"seqin":4 , "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1},
                    "sequence_out":{"seqin":-3, "poscols":['%s_pos'%s1,'%s_pos'%s2], "namecol":"Name", "smode":smode, 'maxk':1}
                }, label_map={'cooperative': 1, 'independent': 0})
            ).run_all(),
        "4":
            BestModel(clf="sklearn.ensemble.RandomForestClassifier",
    ...
    pl.plot_model_metrics(best_models, path="auc_separate_str.pdf", cvfold=10, score_type="auc", varyline=True, title="Average ROC Curves for Ets1-Runx1", interp=True)
